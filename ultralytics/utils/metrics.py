...
# 损失函数分为分类损失和回归损失，分类损失包括交叉熵损失，回归损失其中有WIOU损失，下面是改进针对极小目标的WIOU损失。
class WIoU_Scale:
    ''' monotonous: {
            None: origin v1
    ...
    iou_mean = 1.
    small_target_threshold = 16 * 16  # 定义小目标的面积阈值
    _momentum = 1 - 0.5 ** (1 / 7000)
    _is_train = True
    
    def __init__(self, iou, box_areas):
        self.iou = iou
        self.box_areas = box_areas  # 添加目标面积信息
        self._update(self)
    ...
    def _update(cls, self):
        if cls._is_train: 
            # 分别更新小目标和大目标的IoU均值
            small_mask = self.box_areas < cls.small_target_threshold
            if small_mask.any():
                small_iou_mean = self.iou[small_mask].detach().mean().item()
                cls.small_iou_mean = (1 - cls._momentum) * getattr(cls, 'small_iou_mean', 1.0) + \
                                    cls._momentum * small_iou_mean
            
            large_mask = ~small_mask
            if large_mask.any():
                large_iou_mean = self.iou[large_mask].detach().mean().item()
                cls.large_iou_mean = (1 - cls._momentum) * getattr(cls, 'large_iou_mean', 1.0) + \
                                    cls._momentum * large_iou_mean
    ...
    def _scaled_loss(cls, self, gamma=1.9, delta=3):
        ...
        # 小目标专用缩放因子
        small_beta = self.iou.detach() / getattr(cls, 'small_iou_mean', cls.iou_mean)
        
        if small_mask.any():
            # 对小目标：降低惩罚强度，提高训练稳定性
            alpha_small = delta * 0.7 * torch.pow(gamma, small_beta[small_mask] - delta * 0.8)
            small_scale = small_beta[small_mask] / alpha_small
            
            # 对大目标：保持原有策略
            large_scale = beta[~small_mask] / (delta * torch.pow(gamma, beta[~small_mask] - delta))
            
            combined_scale = torch.zeros_like(beta)
            combined_scale[small_mask] = small_scale
            combined_scale[~small_mask] = large_scale
            
            return combined_scale
        
        return beta / (delta * torch.pow(gamma, beta - delta))
...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, SIoU=False, EIoU=False, WIoU=False, Focal=False, alpha=1, gamma=0.5, scale=False, eps=1e-7):
    ...
    if scale:
        # 传入面积信息用于小目标感知
        self = WIoU_Scale(1 - (inter / union), min_area)
        ...
        if CIoU or DIoU or EIoU or SIoU or WIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                if Focal:
                    ...
                    with torch.no_grad():
                        # 面积加权：小目标对距离更敏感
                        area_ratio = (min_area / (cw * ch + eps)).clamp(0.1, 10.0)
                        distance_sensitivity = 1.0 + (1.0 - area_ratio) * 0.5
                        
                        # 密集度感知：根据重叠程度调整惩罚
                        overlap_ratio = inter / (min_area + eps)
                        density_factor = 1.0 + overlap_ratio * 0.3
                        
                    # 改进的距离惩罚项
                    distance_penalty = torch.exp((rho2 / c2) * distance_sensitivity * density_factor)
                    distance_penalty = distance_penalty.clamp(max=8.0)  # 更严格的梯度控制
                    
                    # 自适应权重调整
                    with torch.no_grad():
                        # 小目标专用权重调整
                        small_target_weight = 1.0 + (min_area < 16*16).float() * 0.3
                        iou_aware_weight = 1.0 - iou.detach().clamp(0, 0.95) * 0.5
                        combined_weight = iou_aware_weight * small_target_weight
                        
                        distance_penalty = distance_penalty * combined_weight
                    
                    # 获取缩放后的损失
                    loss_wiou = getattr(WIoU_Scale, '_scaled_loss')(self)
                    
                    # 返回三元组：缩放损失、距离惩罚、原始IoU
                    return loss_wiou, (1 - iou) * distance_penalty.detach(), iou
